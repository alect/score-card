package org.flixel.system
{
	...
	public class FlxReplay
	{
		...
		/**
		 * Append an empty frame to the replay log.
		 * Useful if we need to know which frame we stopped recording on (like for score verification).
		 */
		public function appendEmptyFrame():void
		{
			var emptyRecord:FrameRecord = (new FrameRecord()).create(frame++);
			_frames[frameCount++] = emptyRecord; 
			if (frameCount >= _capacity) 
			{ 
				_capacity *= 2; 
				_frames.length = _capacity;
